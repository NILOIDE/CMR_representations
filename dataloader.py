...
class CMRDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self) -> None:
        ...
        try:
            ...
            with open("/home/nil/Documents/git/CMR_intensity_alignment/trained_models/20251123-010747-psf40k/checkpoints/dataset_paths_22_unaligned_h5_crop.pkl", 'rb') as handle:
        ...
        subject_data = [Path(self.store_path) / Path(i).parent.name / Path(i).name for i in subject_data]
        assert len(subject_data) == num_subjects
        assert all([Path(i).exists() for i in subject_data])
        shutil.copy("/home/nil/Documents/git/CMR_intensity_alignment/trained_models/20251123-010747-psf40k/checkpoints/dataset_paths_22_unaligned_h5_crop.pkl", Path(self.log_path) / pickle_name)
