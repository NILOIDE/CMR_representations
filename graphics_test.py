...
# seg_path = r"D:\logs\20251125-034718-psf20k_100subj-20ann\pred_volumes\epoch_24000\1011525\pred\full_seg.nii.gz"
...
CREATE_GIF = True
renders = []
render_save_dir = Path('renders')

# for t_prop in [0.0, 0.25, 0.5, 0.75]:
for t_prop in np.arange(0, 10, 1)/10:
    t=int(T*t_prop)
    ...
    # mask = binary_dilation(mask, structure=struct, iterations=25)
    ...
    for label in labels:
        ...
        # ------------------------------------------------------------------------------------
        # Params for 20251125-034718-psf20k_100subj-20ann\pred_volumes\epoch_24000\1011525
        if r"20251125-034718-psf20k_100subj-20ann\pred_volumes\epoch_24000\1011525" in seg_path:
            if t == 0:
                if label == 1:
                    verts += np.array((-1, -2, 0))
                if label == 3:
                    verts += np.array((0.5,0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
                if label == 1:
                    verts *= np.array((0.95, 1, 0.98))
            if t == T//3:
                if label == 1:
                    verts += np.array((-2, -0.5, 0))
                if label == 3:
                    verts += np.array((0.5,0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
                if label == 1:
                    verts *= np.array((0.95, 0.95, 0.98))
            if t == T//3*2:
                if label == 1:
                    verts += np.array((-2, -0.5, 0))
                if label == 3:
                ...
                if label == 1:
                    verts *= np.array((0.95, 0.95, 0.98))
        elif r"20251125-034718-psf20k_100subj-20ann\pred_volumes\epoch_24000\1021869" in seg_path:
            if int(T*0.) <= t < int(T*0.1):
                if label == 3:
                    verts += np.array((1, 0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
                if label == 1:
                    verts *= np.array((0.98, 0.98, 0.98))
                    verts += np.array((-0.01, 0, 0))
            elif int(T*0.1) <= t < int(T*0.4):
                if label == 3:
                    verts += np.array((1, 0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
                if label == 1:
                    verts *= np.array((0.98, 0.98, 0.98))
                    verts += np.array((-0.004, 0, 0))
            elif int(T*0.4) <= t < int(T*0.5):
                if label == 3:
                    verts += np.array((1, 0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
                if label == 1:
                    verts *= np.array((0.98, 0.98, 0.98))
                    verts += np.array((-0.003, 0, 0))
            elif int(T*0.5) <= t < int(T*0.75):
                if label == 3:
                    verts += np.array((1, 0, 0))
                verts = verts / size * 2 - 1
                verts *= np.array((1, 1.3, 1))
    ...
    for label, (v, f, n) in zip(labels, class_meshes):
        ...
        if label == 1:
            clipped = clipped.clip(
                ...
                origin=(0,-0.3,0),      # a point on the plane
    ...
    if SHOW_RENDER:
        fig.show()
    res = (1000,1000)
    save_path = render_save_dir / f"render{t}.png"
    ...
    renders.append(cropped)

if CREATE_GIF:
    # Video parameters
    total_duration = 2.0  # seconds
    fps = len(renders) / total_duration
    output_path = render_save_dir / "render.mp4"
