...
class INR_AutoReg(pl.LightningModule):
    ...
    def forward_inr(self,
        ...
        net_input = self.process_input(coords, subject_latent)
    ...
    def process_input(self,
                    coords: torch.Tensor,
                    subject_latent: torch.Tensor) -> torch.Tensor:
    ...
    def training_step(self, batch):
        opt_inr, opt_deform, opt_inten, opt_regist = self.optimizers()
        opt_inr.zero_grad()
            ...
            for s in range(num_subj_slices.item()):
                ...
                with torch.enable_grad():
                    pred_vals_, pred_seg_, pred_deform_, pred_vals_d_, world_coords_ = self.forward(
                        ...
                        latent_params=latent_params,
                        aff_def_params=aff_def_params,
                        return_deriv=True, return_def=True)
    ...
    def log_volume(self,
        ...
        subj_path = dataset.data_paths[subj_idx]
        subj_id = Path(subj_path).parent.name
        coords = torch.stack(torch.meshgrid(*[torch.linspace(0.15, 0.85, i) for i in res]), dim=-1)
        ims = []
        segs = []
        defs = []
        def_quivers = []
